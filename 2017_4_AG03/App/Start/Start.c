...
u8 UpStart_Init(void)
{
			UpStart_InitStructure.HeatTime		=	3;				//加热时间（HeatTime+1)*0.1ms          
			...
			UpStart_InitStructure.PaperC_EN		=	ENABLE;		//不使能旁路电容检测
			...
			UpStart_InitStructure.Tool_EN		=	DISABLE;	//不使能切刀
			UpStart_InitStructure.VH_High		=	DISABLE;	//使能7200mV高压电源
			UpStart_InitStructure.RAY_High		=	DISABLE;	//高电位使能
